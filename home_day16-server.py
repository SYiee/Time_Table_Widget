...
class Home(QMainWindow, home):
    ...
    def signal3_emitted(self, link):  # 우클릭해서 나온 창(공유하시겠습니까?)에서 OK를 클릭할 때 실행되는 함수 -> 공유하기
        ...
        #print(self.c_name)
        show_text = name + '수업의 zoom 주소를 변경하였습니다.'
        QMessageBox.question(self, 'Message', show_text, QMessageBox.Yes, QMessageBox.NoButton)
        th1 = threading.Thread(target=self.network, args=(self.share_list,))
        th1.start()
        #threading.Timer(60, self.network(link)).start()
    ...
    def network(self,link):
        ...
        self.connectionSock.send((str(link[1])+str(link[2])).encode('utf-8'))
        self.connectionSock.send(str(link[0]).encode('utf-8'))
        #self.connectionSock.send(str(link[2]).encode('utf-8'))
            #time.sleep(1)
